...
cp data/Task07_Pancreas/full_combined_interpolated_points_16.hdf5 $MYTMP
cp data/Task07_Pancreas/full_combined_interpolated_points_200.hdf5 $MYTMP

TORCH_USE_CUDA_DSA=1 CUDA_LAUNCH_BLOCKING=1 python3 train_raygan.py --experiment-name ablation_no_multiscale_curvature_fml --data-path Task07_Pancreas/test_16proj_0 --activation-function relu --num-points 512 --model-lr 1e-3 --d-lr 1e-4 --num-epochs 1000 --batch-size 3500 --num-workers 16 --encoder hashgrid --adversarial-weight 1e-3 --ray-data-path $MYTMP/combined_interpolated_points_16.hdf5 --midpoint 0.15 --sharpness 0.05 --extra-positions-path Task07_Pancreas/200proj_positions.npy --extra-ray-data-path $MYTMP/combined_interpolated_points_200.hdf5 --extra-batch-size 1500 --smoothness-weight 5e-5 --curvature-weight 0 --consistency-weight 5e-2 --fml-weight 1e-3 --dilated --multiscale
