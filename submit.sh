#!/bin/sh
#BSUB -q gpua100
#BSUB -J train
#BSUB -n 20
#BSUB -R "span[hosts=1]"
#BSUB -gpu "num=1:mode=exclusive_process"
#BSUB -W 04:00
#BSUB -R "rusage[mem=8GB]"
##BSUB -R "select[gpu80gb]" #options gpu40gb or gpu80gb
#BSUB -o gpu_logs/gpu_%J.out
#BSUB -e gpu_logs/gpu_%J.err
##BSUB -N
# -- end of LSF options --

nvidia-smi


source ../envs/renner/bin/activate
# source ../miniconda3/bin/activate
# conda activate renner

# Options
# Run main.py --help to get options


TORCH_USE_CUDA_DSA=1 CUDA_LAUNCH_BLOCKING=1 python3 train_model.py --experiment-name synthetic_fibers_angle --model-type mlp --activation-function sine --learning-rate 1e-4 --num-epochs 150 --batch-size 5000 --num-workers 20  --num-points 512 --data-path data/synthetic_fibers_angle/fiber_00 --noisy-points --regularization-weight 1e-1 --noisy-data --noise-std 1e-1
