...
def main(args_dict):
    ...
    if args_dict['training']['imagefit_mode']:
        ...
        if os.path.exists(f"{_PATH_DATA}/{args_dict['general']['data_path']}_latent_vector-{args_dict['model']['latent_size']}.pt"):
            ...
            latent_vector = torch.ones(1, args_dict['model']['latent_size']).normal_(mean=0, std=1 / math.sqrt(args_dict['model']['latent_size'])).cuda()
    ...
    if args_dict['training']['imagefit_mode']:
        wandb_logger = WandbLogger(project="Renner", name=f"{args_dict['general']['experiment_name']}_{args_dict['model']['encoder']}_{args_dict['model']['activation_function']}_latent-size-{args_dict['model']['latent_size']}")
        ...
        wandb_logger = WandbLogger(project="Renner", name=f"{args_dict['general']['experiment_name']}_{args_dict['model']['encoder']}_{args_dict['model']['activation_function']}_regularization-weight-{args_dict['training']['regularization_weight']}_noise-level-{args_dict['training']['noise_level']}_latent-size-{args_dict['model']['latent_size']}")
...
if __name__=="__main__":
    ...
    parser_training.add_argument('--model-lr', type=float, default=1e-5, help='Learning rate of model parameters for the optimizer')
    parser_training.add_argument('--latent-lr', type=float, default=1e-3, help='Learning rate of latent vector for the optimizer')
    ...
    parser_training.add_argument('--full-mode', action='store_true', help='If true train both given latent vector and model parameters. Does nothing if imagefit-mode is on')
