...
class MLP(LightningModule):
    ...
    def __init__(self,args_dict,projection_shape=(300,300),num_volumes=1000,latent=None):
        ...
        self.TV_regularization = tm.image.TotalVariance()
...
class NeuralField(LightningModule):
    ...
    def __init__(self,args_dict,projection_shape=(300,300),num_volumes=1000,latent=None):
        ...
        for i in range(self.num_hidden_layers):
            if i%2==0:
                layers_first_half.append(torch.nn.Sequential(torch.nn.Linear(self.num_hidden_features,self.num_hidden_features),
                                                             get_activation_function(self.activation_function,args_dict),
                                                             torch.nn.Dropout(p=0.2)
                                             ))
            else:
                layers_second_half.append(torch.nn.Sequential(torch.nn.Linear(self.num_hidden_features,self.num_hidden_features),
                                                              get_activation_function(self.activation_function,args_dict),
                                                              torch.nn.Dropout(p=0.2)
                                             ))

        
        self.mlp_first_half = torch.nn.Sequential(torch.nn.Linear(num_input_features,self.num_hidden_features),
                                       get_activation_function(self.activation_function,args_dict),
                                       *layers_first_half,
                                        )
        self.mlp_second_half = torch.nn.Sequential(torch.nn.Linear(self.num_hidden_features+num_input_features,self.num_hidden_features),
                                       get_activation_function(self.activation_function,args_dict),
                                       *layers_second_half,
                                        torch.nn.Linear(self.num_hidden_features,1),
                                        torch.nn.Sigmoid(),
        ...
        self.params = torch.nn.ModuleDict({
            'latent_vectors': torch.nn.ModuleList([self.lat_vecs]),
            'model': torch.nn.ModuleList([self.encoder,self.mlp_first_half,self.mlp_second_half])
        })

        self.loss_fn = torch.nn.MSELoss()
        self.volumefit_loss = torch.nn.MSELoss()
        self.l1_regularization = torch.nn.L1Loss()
        self.psnr = tm.image.PeakSignalNoiseRatio()
        self.TV_regularization = tm.image.TotalVariation(reduction="mean")
        self.validation_step_outputs = []
        self.validation_step_gt = []
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.imagefit_mode:
            ...
            loss += 1e-4*self.TV_regularization(attenuation_values.unsqueeze(dim=1)).cuda()
    ...
    def configure_optimizers(self):
        ...
        if self.imagefit_mode:
            lr_lambda = lambda epoch: 0.98 ** max(0,(epoch-25))
            optimizer = torch.optim.AdamW([
                    {
                        "params": self.params.model.parameters(),
                        "lr": self.model_lr,
