...
class NeuralFieldSingle(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        lr_lambda = lambda epoch: 0.98 ** max(0, (epoch - 50))
...
class MLP_2d(LightningModule):
    ...
    def __init__(self, args_dict, projection_shape, num_volumes):
        ...
        if args_dict["model"]["encoder"] != None:
            self.encoder = get_encoder(
                encoding=args_dict["model"]["encoder"], input_dim=2
            )
            num_input_features = self.encoder.output_dim + self.latent_size
        else:
            self.encoder = None
        ...
        for i in range(self.num_hidden_layers):
            layers.append(
                torch.nn.Sequential(
                    torch.nn.Linear(self.num_hidden_features, self.num_hidden_features),
                    get_activation_function(self.activation_function, args_dict),
                )
            )
        ...
        self.msssim = tm.image.MultiScaleStructuralSimilarityIndexMeasure(
            data_range=1.0
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        msssim_loss = 0.84 * (
            1
            - self.msssim(attenuation_values.unsqueeze(dim=1), target.unsqueeze(dim=1))
        )

        contrast_loss = 1e-3 * (
            1 - (attenuation_values.max() - attenuation_values.min())
        )

        l2_size_loss = torch.sum(torch.norm(self.lat_vecs(idxs), dim=1))
        reg_loss = (1e-4 * min(1, self.current_epoch / 100) * l2_size_loss) / len(idxs)
